...
export default function SearchBar({ searchUrl, links }: Props) {
  ...
  const handleKeyDown = (e: TargetedKeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === "Enter" && selectedIndex >= 0 && selectedIndex < options.length) {
      e.preventDefault();
    ...
    <div class="w-full max-w-2xl">
      <div class="relative">
        ...
        {options.length > 0 && query && (
          <ul id="search-options" class="absolute left-0 right-0 z-50 mt-2 overflow-hidden rounded-2xl frosted-glass">
            {options.map((option, index) => (
              <li key={`${option.name}-${option.url}`}>
                ...
                >
                  ...
                  <span class="truncate text-right text-sm text-white/60">{option.url}</span>
                </button>
              </li>
            ))}
          </ul>
